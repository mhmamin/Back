import orderModel from "../models/orderModels.js"; // تم توحيد الاسم هنا ليطابق بقية الكود
...
const placeOrder = async (req, res) => {
  ...
  try {
    // تم تعديلها هنا أيضاً لتصبح orderModel بدون s
...
export const verifyOrder = async (req, res) => {
  ...
  try {
    if (success === "true") {
      ...
      await createNotificationForOrder(orderData);

      res.json({ success: true, message: "تم الدفع بنجاح" });
    } else {
      await orderModel.findByIdAndDelete(orderId);
      res.json({ success: false, message: "فشلت عملية الدفع" });
    }
  } catch (error) {
    console.log(error);
